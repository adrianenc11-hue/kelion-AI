...
(function () {
    ...
    async function startRealtimeSession() {
        ...
        try {
            ...
            dc.onopen = async () => {
                console.log('🎙️ DataChannel open — voice ready IMMEDIATELY');

                // === NON-BLOCKING GPS: Use cached data if available, update later ===
                let locationInfo = 'Location loading...';
                let weatherInfo = 'Weather loading...';

                // Check if GPS already initialized (auto-init at page load)
                if (window.kelionLocation) {
                    const ctx = window.KelionGPS?.getAIContext?.() || {};
                    locationInfo = ctx.locationInfo || `${window.kelionLocation.city || ''} (${window.kelionLocation.lat}, ${window.kelionLocation.lon})`;
                    weatherInfo = ctx.weatherInfo || 'Weather unknown';
                    console.log('📍 GPS already available:', locationInfo);
                }

                // Send system context NOW — don't wait
                sendEvent({
                    type: 'conversation.item.create',
                    item: {
                        type: 'message',
...
User's location: ${locationInfo}
Current weather: ${weatherInfo}
When user asks about weather or location, use THIS data. Do not make up different data.` }]
                    }
                });

                // If GPS not ready yet, update context in background (NO BLOCKING)
                if (!window.kelionLocation && window.KelionGPS) {
                    window.KelionGPS.init().then(() => {
                        const ctx = window.KelionGPS.getAIContext();
                        sendEvent({
                            type: 'conversation.item.create',
                            item: {
                                ...
                                role: 'system',
                                content: [{
                                    type: 'text', text: `[UPDATED LOCATION - USE THIS NOW]
User's EXACT location: ${ctx.locationInfo}
Current weather: ${ctx.weatherInfo}`
                                }]
                            }
                        });
                        console.log('📍 GPS context updated in background');
                    }).catch(e => console.warn('📍 Background GPS failed:', e.message));
                }
            };
    ...
    async function handleRealtimeEvent(raw) {
        ...
        // User transcript (what user said) - TOTUL TRECE PRIN SMART BRAIN
        if (evt.type === 'conversation.item.input_audio_transcription.completed' && evt.transcript) {
            ...
            // OPREȘTE GPT-4o din a răspunde - SMART BRAIN preia controlul
            interruptNow('brain-takeover');

            // TOTUL TRECE PRIN CHAT.JS — CREIERUL REAL CU 69 FUNCȚII
            console.log('🧠 BRAIN: Trimite la chat.js (69 tools reale)...');
            try {
                // Construiesc mesajul cu context real (locație + vreme)
                let contextPrefix = '';
                if (window.kelionLocation?.city) {
                    contextPrefix += `[Locația mea: ${window.kelionLocation.address || window.kelionLocation.city}] `;
                }
                if (window.kelionWeather) {
                    contextPrefix += `[Vremea: ${window.kelionWeather.temp}°C, ${window.kelionWeather.description}] `;
                }

                const brainResponse = await fetch('/.netlify/functions/chat', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        message: contextPrefix + evt.transcript
                    })
                });

                const brainData = await brainResponse.json();
                const answer = brainData.response || brainData.reply || brainData.answer || 'Nu am putut procesa cererea.';

                console.log('🧠 BRAIN răspunde:', answer);
                addMessageToChat('assistant', answer);

                // Execută acțiunile UI de la Brain (dacă chat.js returnează)
                if (brainData.actions && brainData.actions.length > 0) {
                    executeBrainActions(brainData.actions);
                }

                // Trimite răspunsul în OpenAI Realtime ca să-l vorbească K
                sendEvent({
                    type: 'conversation.item.create',
                    item: {
                        type: 'message',
    ...
    // ============ EXECUTE BRAIN UI ACTIONS ============
    // Acțiuni UI primite de la supreme-brain.js
    function executeBrainActions(actions) {
        if (!actions || !Array.isArray(actions)) return;

        console.log('🎯 Executing Brain actions:', actions);
                ...
                switch (action.type) {
                    case 'openWorkspace':
                        if (window.kWorkspace) {
                            ...
                            console.log('🖥️ Workspace opened via Brain');
                        }
                        break;
                    ...
                    case 'showLocation':
                        if (window.kelionLocation) {
                            showLocationWorkspace(window.kelionLocation.lat, window.kelionLocation.lon, 'Locația ta');
                        } else {
                            console.warn('📍 Location failed: GPS data not available (pre-fetch not completed)');
                        }
                        break;

                    case 'navigate':
                        if (action.destination && window.kelionLocation) {
                            handleNavigation(action.destination, window.kelionLocation.lat, window.kelionLocation.lon, 'driving');
                        } else if (action.destination) {
                            console.warn('🗺️ Navigation failed: GPS data not available');
                        }
                        break;
    ...
    async function brainOrchestrator(userMessage) {
        ...
        try {
            // === PASUL 0: FOLOSEȘTE GPS DIN CACHE (pre-fetch la session start) ===
            console.log('📍 BRAIN: Citesc GPS din cache (window.kelionLocation)...');
            let realLat = null, realLon = null, realCity = 'necunoscut';
            if (window.kelionLocation) {
                realLat = window.kelionLocation.lat || window.kelionLocation.latitude;
                realLon = window.kelionLocation.lon || window.kelionLocation.longitude;
                realCity = window.kelionLocation.city || 'locația ta';
                console.log('📍 BRAIN: GPS din cache:', realLat, realLon, realCity);
            } else {
                console.error('📍 BRAIN: GPS cache gol - pre-fetch nu s-a finalizat');
                return {
    ...
    async function showWeatherWorkspace(inputLat, inputLon, weatherInfo) {
        ...
        // Dacă nu avem coordonate, folosește cache-ul GPS (pre-fetch la session start)
        if (!lat || !lon) {
            if (window.kelionLocation) {
                ...
                console.log('📍 GPS din cache:', lat, lon);
            } else {
                console.error('📍 GPS cache gol - nu pot afișa harta fără coordonate');
                return;
            }
        }
    ...
    // Get user location with full address (street + number) — uses cached GPS data
    async function getLocationInfo() {
        if (window.kelionLocation) {
            return JSON.stringify(window.kelionLocation);
        }
        return JSON.stringify({ error: 'Location not available', message: 'GPS pre-fetch not completed' });
    }
    ...
    // Afișează harta cu poziția curentă în workspace — folosește GPS cache
    async function showMyLocationMap() {
        console.log('📍 Showing my location map in workspace (from cache)');

        if (!window.kelionLocation) {
            console.error('📍 Location map error: GPS cache gol');
            return 'Nu am putut obține locația: GPS pre-fetch nu s-a finalizat';
        }
        ...
        const address = window.kelionLocation.address || window.kelionLocation.full_address || `${lat.toFixed(6)}, ${lon.toFixed(6)}`;
        ...
        return `Locație afișată: ${address}`;
    }
