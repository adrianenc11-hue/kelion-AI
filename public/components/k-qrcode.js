    ...
    window.K_generateQR = async function (data, opts = {}) {
        ...
        const task = window.K_TASKS?.add(`📱 QR: ${(data || '').substring(0, 20)}`, async (onProgress) => {
            ...
            panel.innerHTML = `
                ...
                ${description ? `<p style="color:rgba(255,255,255,0.5);font-size:0.85rem;margin-bottom:16px;">${description}</p>` : ''}
                <div id="k-qr-canvas" style="display:inline-block;padding:16px;background:#fff;border-radius:12px;margin-bottom:16px;"></div>
                <div style="margin-top:8px;">
                    <p style="color:rgba(255,255,255,0.4);font-size:0.75rem;word-break:break-all;margin-bottom:12px;">${data}</p>
                    <div style="display:flex;gap:8px;justify-content:center;">
                        ...
                        <button onclick="window._kqrCopy('${data.replace(/'/g, "\\'")}')" style="padding:8px 16px;border-radius:10px;
