...
const KelionSubscription = {
    ...
    _escHtml(s) { return String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;'); },
    ...
    updateLoginButton(email) {
        ...
        if (btn) {
            ...
            if (isAdmin) {
                ...
                btn.innerHTML = `✓ ${this._escHtml(shortEmail)} (Logout)`;
    ...
    revealK(isSubscribed = false) {
        ...
        // Auto-start realtime voice and vision - ALL go through Smart Brain
        setTimeout(() => {
            // Start realtime voice
            if (window.kelionRealtime && !window.kelionRealtime.isActive()) {
                console.log('🎙️ Starting voice (Smart Brain handles responses)...');
                window.kelionRealtime.start();
            }
            // Start vision (camera-based presence detection) - goes through Smart Brain
            if (window.kelionVision && !window.kelionVision.isEnabled()) {
                console.log('👁️ Starting vision (Smart Brain handles)...');
                window.kelionVision.init();
            }
        }, 1500);
        console.log('🧠 SMART BRAIN is the central brain - all AI goes through Smart Brain');
    },

    // Choose FREE TRIAL - show age gate first, then reveal K with timer
    chooseFreeTriak() {
        // Check if age already verified this session
        if (sessionStorage.getItem('kelion_age_verified')) {
            this.revealK(false);
            return;
        }
        this.showAgeGate();
    ...
    showAgeGate() {
        // Remove choice modal first
        const choiceModal = document.getElementById('kelion-choice-modal');
        if (choiceModal) choiceModal.remove();

        const existing = document.getElementById('kelion-age-gate');
        if (existing) existing.remove();

        const modal = document.createElement('div');
        modal.id = 'kelion-age-gate';
        modal.innerHTML = `
            <div class="age-overlay"></div>
            <div class="age-modal">
                <div class="age-icon">🛡️</div>
                <h2>Age Verification</h2>
                <p class="age-desc">You must be at least <strong>18 years old</strong> to use Kelion AI.</p>
                <p class="age-question">Are you 18 or older?</p>
                <div class="age-buttons">
                    <button onclick="KelionSubscription.confirmAge(true)" class="age-btn age-yes">
                        ✅ Yes, I am 18+
                    </button>
                    <button onclick="KelionSubscription.confirmAge(false)" class="age-btn age-no">
                        ❌ No, I am under 18
                    </button>
                </div>
                <p class="age-legal">By confirming, you agree to our <a href="terms.html">Terms of Service</a> and <a href="privacy.html">Privacy Policy</a>.</p>
            </div>
        `;
    ...
    confirmAge(isAdult) {
        const gate = document.getElementById('kelion-age-gate');
        if (isAdult) {
            sessionStorage.setItem('kelion_age_verified', 'true');
            if (gate) gate.remove();
            this.revealK(false); // Not subscribed = has timer
        } else {
            // Under 18 - show blocked message
            if (gate) {
                const modalDiv = gate.querySelector('.age-modal');
                if (modalDiv) {
                    modalDiv.innerHTML = `
                        <div class="age-icon">🚫</div>
                        <h2>Access Restricted</h2>
                        <p class="age-desc">Sorry, Kelion AI is only available for users aged 18 and above.</p>
                        <p class="age-desc">Please come back when you're old enough!</p>
                        <button onclick="window.location.href='/'" class="age-btn age-back">
                            🏠 Return to Home
                        </button>
                    `;
                }
    ...
    // Add age gate styles
    addAgeGateStyles() {
        if (document.getElementById('kelion-age-styles')) return;
        const style = document.createElement('style');
        style.id = 'kelion-age-styles';
        style.textContent = `
            #kelion-age-gate .age-overlay {
                position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
                background: rgba(0,0,0,0.92); z-index: 99999;
            }
            #kelion-age-gate .age-modal {
                position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
                background: linear-gradient(135deg, #1a1a2e 0%, #0a0a1a 100%);
                border: 2px solid #d4af37; border-radius: 25px; padding: 40px;
                max-width: 420px; width: 90%; z-index: 100000; text-align: center; color: #fff;
                box-shadow: 0 25px 80px rgba(212,175,55,0.3);
            }
            #kelion-age-gate .age-icon { font-size: 3rem; margin-bottom: 10px; }
            #kelion-age-gate h2 { color: #d4af37; font-family: 'Cinzel', serif; font-size: 1.6rem; margin: 0 0 10px; }
            #kelion-age-gate .age-desc { color: #aaa; font-size: 0.95rem; margin: 0 0 8px; line-height: 1.5; }
            #kelion-age-gate .age-desc strong { color: #ffd700; }
            #kelion-age-gate .age-question { color: #fff; font-size: 1.1rem; font-weight: 600; margin: 15px 0 20px; }
            #kelion-age-gate .age-buttons { display: flex; flex-direction: column; gap: 12px; }
            #kelion-age-gate .age-btn {
                width: 100%; padding: 16px; border-radius: 14px; font-size: 1.05rem;
                font-weight: 600; cursor: pointer; transition: all 0.3s; border: 2px solid #d4af37;
                font-family: inherit; min-height: 48px; touch-action: manipulation;
            }
            #kelion-age-gate .age-yes {
                background: linear-gradient(180deg, #1a5a3a 0%, #0d3d28 100%); color: #ffd700;
            }
            #kelion-age-gate .age-yes:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(26,90,58,0.4); }
            #kelion-age-gate .age-no {
                background: linear-gradient(180deg, #5a1a1a 0%, #3d0d0d 100%); color: #ff6b6b;
                border-color: #ff6b6b;
            }
            #kelion-age-gate .age-no:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(90,26,26,0.4); }
            #kelion-age-gate .age-back {
                background: linear-gradient(180deg, #1a3a6c 0%, #0a2a5c 100%); color: #d4af37;
                margin-top: 20px;
            }
            #kelion-age-gate .age-back:hover { transform: translateY(-2px); }
            #kelion-age-gate .age-legal { color: #555; font-size: 0.72rem; margin-top: 18px; }
            #kelion-age-gate .age-legal a { color: #d4af37; text-decoration: none; }
            #kelion-age-gate .age-legal a:hover { text-decoration: underline; }
        `;
        document.head.appendChild(style);
    },
        ...
        modalContent.innerHTML = `
            ...
            <form class="login-form" onsubmit="event.preventDefault(); KelionSubscription.sendResetLink();">
                <div class="input-group">
                    ...
                    <div class="input-wrapper">
                        <span class="input-icon">📧</span>
                        <input type="email" id="reset-email" value="${this._escHtml(existingEmail)}" placeholder="your@email.com" autocomplete="email" required>
                    </div>
                </div>
    ...
    showAccountMenu() {
        ...
        menu.innerHTML = `
            ...
            <div style="position:fixed;top:70px;left:20px;background:#1a1a2e;border:1px solid #d4af37;border-radius:10px;padding:15px;z-index:99999;min-width:200px;">
                ...
                <p style="color:#fff;margin:0 0 15px;font-weight:bold;">${this._escHtml(email)}</p>
