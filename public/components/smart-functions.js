...
class SmartFunctionExecutor {
    ...
    async showWeatherMap(lat, lon, info) {
        // Get GPS from cache if not provided
        if (!lat || !lon) {
            if (window.kelionLocation) {
                ...
                throw new Error('GPS data not available (pre-fetch not completed)');
            }
        }
    ...
    async showMyLocation() {
        ...
        if (window.KWorkspacePanel) {
            window.KWorkspacePanel.showLocation(lat, lon, 'Your Location');
        }
    ...
    async getLocation() {
        if (!window.kelionLocation) {
            return { success: false, error: 'GPS data not available (pre-fetch not completed)' };
        }
        return {
            success: true,
            lat: window.kelionLocation.lat || window.kelionLocation.latitude,
