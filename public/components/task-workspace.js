...
print(f"Welcome, {name}!")</textarea>
        ...
        async generateImage() {
            ...
            try {
                ...
                if (data.success && data.imageUrl) {
                    ...
                    result.innerHTML = `<p style="color:#ff6b6b;">Error: ${data.error || 'Failed to generate'}</p>`;
                }
            } catch (error) {
            ...
            // For Word files - use Mammoth.js
                ...
                reader.onload = async function (e) {
                    try {
                        ...
                        viewer.innerHTML = `<div style="padding:40px;text-align:center;color:#ff6b6b;">❌ Error: ${err.message}</div>`;
                    }
                };
        ...
        async openUSBCamera() {
            ...
            try {
                ...
                viewer.innerHTML = `<div style="padding:40px;text-align:center;color:#ff6b6b;">❌ Eroare cameră: ${error.message}<br><br>Verifică dacă microscopul USB este conectat.</div>`;
            }
        }
        ...
        useMyLocation() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition((pos) => {
                    ...
                    fromInput.value = `${pos.coords.latitude},${pos.coords.longitude}`;
        ...
        async handleZipUpload(event) {
            ...
            try {
                ...
                list.innerHTML = `<div style="padding:20px;color:#ff6b6b;">Error reading ZIP: ${error.message}</div>`;
            }
        }
        ...
        async showWeatherMap(lat, lon, weatherData = null) {
            ...
            if (!lat || !lon) {
                if (window.kelionLocation) {
                    lat = window.kelionLocation.latitude;
                    lon = window.kelionLocation.longitude;
                    ...
                    try {
                        ...
                        lat = 44.4268; lon = 26.1025; // Default: Bucharest
                    }
                }
            }
