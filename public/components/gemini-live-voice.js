...
Never make up information - use tools or say "I'll search for that."`;
    ...
    async handleFunctionCall(functionCall) {
        ...
        let result;
        try {
            // Route complex queries through Smart Brain
            if (['web_search', 'verify_fact', 'deep_research'].includes(functionCall.name)) {
                result = await this.callSmartBrain(functionCall);
            } else {
    ...
    async callSmartBrain(functionCall) {
    ...
    async callLocation() {
        ...
        return { error: 'GPS data not available (pre-fetch not completed)' };
    }
