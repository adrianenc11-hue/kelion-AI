...
// GPT-4o primary → Gemini Flash → DeepSeek → Kimi
...
exports.handler = async (event) => {
    ...
    try {
        ...
        const researchPrompt = `Cercetează în profunzime: ${topic}. Oferă analize detaliate, structurate, cu perspective multiple, date concrete, și concluzii bine fundamentate.`;
        ...
        if (mode === 'parallel' || mode === 'multi') {
            const results = await Promise.allSettled([
                ...
                researchKimi(topic, systemPrompt)
            ]);
        ...
        const engines = [
            ...
            { key: 'KIMI_API_KEY', fn: () => researchKimi(topic, systemPrompt) }
...
async function researchKimi(topic, systemPrompt) {
    const res = await fetch('https://api.moonshot.cn/v1/chat/completions', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${process.env.KIMI_API_KEY}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: 'moonshot-v1-8k', messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: `Cercetează: ${topic}` }], max_tokens: 3000 })
    });
    if (!res.ok) throw new Error(`Kimi ${res.status}`);
    const data = await res.json();
    return { engine: 'kimi-k2.5', research: data.choices?.[0]?.message?.content, model: 'moonshot-v1-8k' };
}
