...
exports.handler = async (event) => {
    ...
    try {
        ...
        const { text, topic, style, language, action } = JSON.parse(event.body || '{}');

        // ═══ STATUS ACTION — Lightweight health probe (no generation) ═══
        if (action === 'status') {
            return {
                statusCode: 200, headers, body: JSON.stringify({
                    status: 'ready',
                    engine: 'openai-tts',
                    supported_styles: ['conversational', 'educational', 'news', 'storytelling'],
                    tts_model: 'tts-1',
                    voice: 'onyx',
