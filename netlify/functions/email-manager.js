...
exports.handler = async (event) => {
    ...
    // Load vault secrets BEFORE any env var reads
    ...
    if (event.httpMethod === 'GET') {
        ...
        if (params.action === 'oauth_callback' && params.code) {
            ...
            if (tokens.error) {
                const safeDesc = (tokens.error_description || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
                return { statusCode: 400, headers: { 'Content-Type': 'text/html' }, body: `<h1>Error: ${safeDesc}</h1>` };
            }
