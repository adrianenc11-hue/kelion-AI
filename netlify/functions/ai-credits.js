...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (process.env.KIMI_API_KEY) {
            try {
                const testRes = await fetch('https://api.moonshot.cn/v1/models', {
                    headers: { 'Authorization': `Bearer ${process.env.KIMI_API_KEY}` }
                });
                credits.kimi = { status: testRes.ok ? 'active' : 'error', configured: true, provider: 'Moonshot (Kimi)', models: ['moonshot-v1-8k', 'moonshot-v1-32k'], note: 'Pay-per-use' };
            } catch (e) {
                credits.kimi = { status: 'error', configured: true, error: e.message, provider: 'Moonshot (Kimi)' };
            }
        } else {
            credits.kimi = { status: 'missing', configured: false, provider: 'Moonshot (Kimi)' };
        }
